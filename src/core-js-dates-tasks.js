...
function getNextFriday(date) {
  ...
  const secondsPerDay = 8.64e7;
  let daysToNextFriday = 7;
  if (weekday < 5) {
    daysToNextFriday = 5 - weekday;
  }
  if (weekday > 5) {
    daysToNextFriday = weekday;
  }
  return new Date(+date + daysToNextFriday * secondsPerDay);
}
...
function getCountDaysOnPeriod(dateStart, dateEnd) {
  const secondsPerDay = 8.64e7;
  return (
    (new Date(dateEnd) - new Date(dateStart) + secondsPerDay) / secondsPerDay
  );
}
...
function getCountWeekendsInMonth(month, year) {
  ...
  for (let date = 1; date <= daysInMonth; date += 1) {
    const day = new Date(year, monthInd, date);
...
function getWeekNumberByDate(date) {
  ...
  const numberOfDays = Math.floor((date - firstJan) / secondsPerDay);
  let numOfFirstWeekDays = 7 - (firstJan.getDay() - 1);
  if (firstJan.getDay() === 0) {
    numOfFirstWeekDays = 1;
  }
  if (numberOfDays - numOfFirstWeekDays <= 0) {
    result = 1;
  } else {
    result = Math.ceil((numberOfDays - numOfFirstWeekDays + 1) / 7) + 1;
  }
  return result;
}
...
function getNextFridayThe13th(date) {
  ...
  const resultFunc = (startMonth, startYear) => {
    for (let monthInd = startMonth; monthInd < 12; monthInd += 1) {
      const d = new Date(startYear, monthInd, 13);
      if (d.getDay() === 5) {
        return d;
      }
    }
    return resultFunc(0, startYear + 1);
  };
  return resultFunc(month, year);
}
...
function getWorkSchedule(period, countWorkDays, countOffDays) {
  ...
  const startTime = new Date(
    [start[1], start[0], start[2]].join('-')
  ).getTime();
  const endTime = new Date([end[1], end[0], end[2]].join('-')).getTime();
  for (
    let k = startTime;
    k <= endTime;
    k += (countWorkDays + countOffDays) * secondsPerDay
  ) {
    for (let t = k; t < k + countWorkDays * secondsPerDay; t += secondsPerDay) {
      if (t > endTime) break;
      const workDay = new Intl.DateTimeFormat('en-GB').format(new Date(t));
      workSchedule.push(workDay.split('/').join('-'));
    }
  }
  return workSchedule;
}
